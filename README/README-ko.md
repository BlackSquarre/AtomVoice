...
### ⚡ 가벼운 설계
설치 패키지는 3 MB 미만이고 백그라운드 데몬이 없습니다. Sherpa-ONNX 로컬 인식은 CoreML 백엔드를 사용해 CPU 부하와 전력 사용을 줄일 수 있습니다. Sherpa 런타임, ASR 모델, 문장 부호 모델은 필요할 때 다운로드되며, 큰 로컬 모델은 유휴 시간 이후나 심각한 메모리 압박 시 해제할 수 있습니다.

### ⌨️ 입력 습관과 호환
AtomVoice는 시스템 입력기를 대신 차지하지 않고 기존 타이핑 습관을 바꾸지 않습니다. 익숙한 한국어/영어 입력기, 단축키, 설정을 그대로 사용할 수 있습니다. 치고 싶을 때는 치고, 말하고 싶을 때는 말하세요. 긴 문장, 추가 설명, 여러 앱을 오가는 입력에서도 언제든 음성 입력으로 이어갈 수 있습니다.
...
### 녹음과 트리거
- **눌러서 말하기 / 한 번 눌러 말하기** — 원하는 모드를 선택, 인식 텍스트 기반 무음 자동 정지와 결합해 조용히 말하거나 주변이 시끄러울 때의 오탐 정지를 줄일 수 있음
- **트리거 키 사용자 지정** — 자기 키보드에 맞는 수정자 키 선택
- **녹음 중 단축키** — 한 번에 취소, LLM 건너뛰고 즉시 삽입, 또는 지정한 문장 부호로 마무리
- **헤드폰 음성 제어 (Beta)** — 헤드폰 재생/일시정지 버튼으로 음성 입력 제어; USB 헤드셋 / DAC 제어 버튼과 3.5 mm 헤드셋 버튼에서 검증됨: 한 번 누름은 현재 입력 모드 따름, 길게 누름은 녹음, 두 번 누름은 Return 전송
- **앱 전환 시 자동 취소**(눌러서 말하기 모드에서만)

### 인식 엔진
- **Apple 음성 인식** — 시스템 엔진, 스트리밍과 선택적 기기 내 인식, SFSpeechRecognizer의 1분 한계를 넘는 **롤링 분할** 지원
- **Sherpa-ONNX** — 완전 오프라인 로컬 엔진, 언어별 모델 프리셋, CPU/CoreML 백엔드, 런타임/ASR/문장 부호 모델 온디맨드 다운로드, 자동 언로드, 서드파티 모델 가져오기 지원
...
- **녹음 중 시스템 볼륨 낮추기**(옵션)
- **단일 인스턴스 보호** — 시작 시 이전 인스턴스 자동 종료, 여러 AtomVoice 인스턴스가 헤드폰 버튼 이벤트를 동시에 잡는 상황 감소
...
`make lint-loc`는 `loc("key")` 사용과 8개 현지화 디렉터리를 비교해 UI 문구 번역 누락을 찾아냅니다.
...
- **Sherpa-ONNX**는 **인식 엔진 설정 → Sherpa 로컬**에서 설정합니다. 언어, 모델 프리셋, CPU/CoreML 백엔드, 자동 언로드 지연 시간을 선택하거나 서드파티 모델 패키지를 가져올 수 있습니다.
