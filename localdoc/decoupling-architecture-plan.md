...
- 完成阶段 5 小阶段：新增 `Sources/AtomVoice/RecognitionResultFinalizer.swift`，集中最终识别结果收尾流程。
- `RecognitionResultFinalizer` 负责 raw text 到最终上屏 action 的决策：Apple live insertion 剩余文本裁剪、自动标点、LLM 润色、streaming replacement、immediate stop 标点追加、空文本/error fallback 和一次性 deliver。
- `RecordingSessionController` 的 `finishRecording` / `finishImmediateRecording` 分支显著变薄，只组装 finalization request，并继续保留录音状态机、ASR 停止和 Doubao fallback 编排职责。
- 新增测试覆盖 finalizer 的 paste 输出、streaming replacement、immediate punctuation、LLM 成功、LLM 失败、空文本错误 fallback、Apple live insertion 剩余文本处理和 streaming + live insertion 下的 LLM 替换。
- 验证：`make test` 已执行，但当前 Codex 外层沙盒拒绝 SwiftPM 内部 `sandbox-exec`，在 manifest 阶段以 `sandbox_apply: Operation not permitted` 停止；随后用同一测试入口 `swift run --disable-sandbox -Xswiftc -enable-testing AtomVoiceArchitectureTests` 通过 24 条架构测试。
- 耳机线控安全识别补充：新增 `HeadphoneHIDSourceMonitor` / `HeadphoneHIDSourceClassifier`，用 IOHID Consumer Control 来源证明 gate 现有 `HeadphoneMonitor` 的 `NX_KEYTYPE_PLAY` 路径；默认拒绝键盘、AirPods 名称和未知来源，受限接受命名明确的 `transport=Audio` 有线 headset control。
- 耳机交互时序补充：`HeadphoneInputCoordinator` 对“未录音 + 单击说话模式”的可信 Play/Pause 做乐观录音，`RecordingSessionController` 支持延迟胶囊呈现；等待双击窗口期间音频和 ASR 先启动，胶囊 / 流式上屏 / Apple live insertion 延后，双击成立则取消并丢弃等待期结果。
- 验证：`make test` 通过 35 条架构测试，新增覆盖可信 USB Consumer Control、键盘 Consumer Control、未知 transport、AirPods 名称、键盘属性提示、模糊 USB receiver 名称拒绝、命名明确的 Audio headset control 接受和泛化 Audio Consumer Control 拒绝。
- 下一步：进入 `RecordingSessionPresenter`，把胶囊 UI 的 recording / progress / error / refining / text update / dismiss / shimmer / bands update 从 session 状态机中继续抽出。
