...
### 开发调试
```bash
make dev        # 编译并安装到 dist/Test/VoiceInput.app（供确认后使用）
...
2. **用户确认版本号后**，同步修改两处：
   - `Sources/VoiceInput/Info.plist` → `CFBundleShortVersionString`
...
4. 用户确认发版后，依次执行：
   ...
   gh release create vX.Y.Z dist/VoiceInput-X.Y.Z-*.zip --title "vX.Y.Z" --generate-notes
   ```
